...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Options processing (example for MCQs - adjust based on selected question type)
    // This part needs to be more dynamic based on JS or multiple submit stages
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            // Insert options if it's an MCQ type (assuming type ID 1 is MCQ, adjust as per your DB)
            ...
            // Add True/False options automatically if that type is selected
                ...
                // Add True option (is_correct will be set by user if this is the right answer for the question)
            ...
            if (!empty($tags_input)) {
                ...
                foreach ($tag_ids_to_link as $tag_id) {
                    ...
                    if (!$stmt_link_tag->execute()) {
                        // Ignore duplicate errors for linking, or handle more gracefully
...
<form action="add_question.php" method="post" id="addQuestionForm">
    ...
    <div id="options_container" style="display: none;"> <!-- Hidden by default, shown by JS for MCQs -->
        <h4>Options:</h4>
        <div id="mcq_options_list">
            <!-- Option template -->
            <div class="mcq_option_item" style="margin-bottom: 10px;">
                <input type="text" name="options[0][text]" placeholder="Option text">
                <label><input type="checkbox" name="options[0][is_correct]" value="1"> Correct</label>
                <button type="button" class="remove_option_btn">Remove</button>
            </div>
        </div>
        <button type="button" id="add_mcq_option_btn">Add Option</button>
    </div>

    <div id="true_false_container" style="display: none;"> <!-- Hidden by default, shown for True/False -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let optionCounter = 1; // Start from 1 because one is already in HTML (template)
    ...
    function toggleOptionInputs() {
        ...
        if (typeName === 'multiple choice') {
            ...
            if(mcqOptionsList.children.length === 0) addMcqOption(); // Add one if empty
    ...
    // Initial check in case of page reload with selected type
    toggleOptionInputs();

    // Event delegation for remove buttons on dynamically added options
    ...
    // Remove the template option if it's still there and not needed
    ...
    if(initialTemplate && mcqOptionsContainer.style.display === 'none') {
        // initialTemplate.remove(); // Or just ensure it's not submitted if not MCQ
    } else if (initialTemplate && mcqOptionsContainer.style.display === 'block' && mcqOptionsList.children.length > 1) {
        // If we auto-added one, and the template exists, this logic might need refinement
        // For now, the first one acts as the first dynamic item if MCQ is selected.
    }
